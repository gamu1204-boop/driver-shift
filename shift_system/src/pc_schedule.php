...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_schedule_all'])) {
    ...
    if ($jsonData !== null) {
        ...
        echo "<script>location.href='pc_schedule.php?saved=1';</script>";
        exit;
    }
}
...
$activeDrivers = array_filter($allDrivers, function($d) { 
    return (!isset($d['is_deleted']) || $d['is_deleted'] === false) && (isset($d['is_active']) && $d['is_active'] == 1); 
});
...
<head>
    ...
    <style>
        ...
        .course-item { background-color: #007bff; color: white; padding: 4px 8px; margin: 2px; border-radius: 3px; cursor: move; display: inline-block; font-size: 11px; }
        .v-input { width: 90%; font-size: 12px; text-align: center; margin-top: 5px; border: 1px solid #ccc; padding: 2px; border-radius: 3px; }
        .btn-save { background-color: #28a745; color: white; padding: 12px 50px; border: none; border-radius: 4px; cursor: pointer; font-size: 18px; font-weight: bold; }
        .escape-area { position: fixed; top: 10px; right: 20px; width: 200px; background-color: rgba(255, 243, 205, 0.98); border: 2px solid #ffeeba; padding: 10px; border-radius: 10px; z-index: 9999; }
        .quick-items { display: flex; flex-wrap: wrap; gap: 4px; }
        .quick-item { background-color: #6c757d; font-size: 10px; padding: 3px 6px; }
        .quick-item#quick-large { background-color: #1565c0; }
...
<div class="container">
    ...
    <div class="navigation">
        <a href="pc_schedule.php?year=<?= (clone $monday)->modify('-7 days')->format('Y') ?>&month=<?= (clone $monday)->modify('-7 days')->format('m') ?>&day=<?= (clone $monday)->modify('-7 days')->format('d') ?>">前の週</a>
        <strong style="margin: 0 20px;"><?= $monday->format('Y/m/d') ?> 〜 <?= (clone $monday)->modify('+6 days')->format('Y/m/d') ?></strong>
        <a href="pc_schedule.php?year=<?= (clone $monday)->modify('+7 days')->format('Y') ?>&month=<?= (clone $monday)->modify('+7 days')->format('m') ?>&day=<?= (clone $monday)->modify('+7 days')->format('d') ?>">次の週</a>
    </div>

    <div class="escape-area">
        <p style="font-size:11px; margin:0 0 8px 0; font-weight:bold;">📦 一時置き場・よく使う項目</p>
        <div class="quick-items">
            <div class="course-item quick-item" draggable="true" ondragstart="drag(event)" id="quick-large">大型配送</div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <?php foreach ($dates as $day): 
                        ...
                        // 会社が休業日の場合は自動的に「公休」
                        if (!$day['is_working'] && !$saved) {
                            $rawCourse = '公休';
                        } else {
                            $rawCourse = $saved['course'] ?? ($driver['courses'][$day['dayKey']]['course'] ?? '-');
                        }
                    ...
                    ?>
                    <td class="drop-zone" data-driver-id="<?= $driverId ?>" data-date="<?= $dKey ?>" ondrop="drop(event)" ondragover="allowDrop(event)">
                        <?php if ($courseName !== '-' && $courseName !== ''): ?>
